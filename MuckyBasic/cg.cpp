...
std::int32_t CG_do(char* fname, std::int32_t output) {
    ...
    if (output & CG_OUTPUT_EXECUTABLE) {
        ...
        if (fwrite(CG_instruction, sizeof(std::int32_t), CG_instruction_upto, handle) != CG_instruction_upto)
            goto file_error;
        ...
        if (fwrite(CG_data_table, sizeof(std::uint8_t), CG_data_table_upto, handle) != CG_data_table_upto)
            goto file_error;
...
#define CG_WRITE_MAGIC()                                          \
    {                                                             \
        if (fwrite(&magic, sizeof(std::int32_t), 1, handle) != 1) \
            goto file_error;                                      \
    }
        ...
        if (fwrite(&CG_debug_data_upto, sizeof(std::int32_t), 1, handle) != 1)
            goto file_error;
        if (fwrite(CG_debug_data, sizeof(char), CG_debug_data_upto, handle) != CG_debug_data_upto)
            goto file_error;
