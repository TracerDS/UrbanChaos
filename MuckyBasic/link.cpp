...
#define LINK_MAGIC_CHECK()                                       \
    {                                                            \
        if (fread(&magic, sizeof(std::int32_t), 1, handle) != 1) \
            goto file_error;                                     \
    ...
    for (i = 0; i < num_object_files; i++) {
        ...
        if (fread(&lf->num_debug_data, sizeof(std::int32_t), 1, handle) != 1)
            goto file_error;
    ...
    if (fwrite(LINK_instruction, sizeof(std::int32_t), LINK_instruction_upto, handle) != LINK_instruction_upto)
        goto file_error;
    ...
    if (fwrite(LINK_table_data, sizeof(std::uint8_t), LINK_table_data_upto, handle) != LINK_table_data_upto)
        goto file_error;
