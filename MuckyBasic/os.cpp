...
std::int32_t OS_poly_count_transformed;
std::int32_t OS_poly_count_drawn;
...
volatile std::uint8_t KEY_on[256];
volatile std::int32_t KEY_inkey;
volatile std::int32_t KEY_shift;
...
found_spare_sound:;
    ...
    pwfx.nAvgBytesPerSec = 22050 * sizeof(std::uint16_t);
    pwfx.nBlockAlign = 2;
    ...
    dsbdesc.dwSize = sizeof(DSBUFFERDESC);
    dsbdesc.dwFlags = DSBCAPS_STATIC;
    dsbdesc.dwBufferBytes = num_samples * sizeof(std::uint16_t);
    dsbdesc.lpwfxFormat = &pwfx;
...
char OS_texpath[OS_MAX_TEXPATHS][64];
std::int32_t OS_texpath_upto;
std::int32_t OS_texpath_last; // The last texture path we used successfully.
...
std::int32_t OS_message_handler_request_quit;
...
std::int32_t OS_fps;
std::int32_t OS_last_time;
std::int32_t OS_last_frame_count;
std::int32_t OS_frame_count;

std::int32_t OS_fps_get() {
    std::int32_t now;
...
std::uint32_t OS_interpolate_colour(float v, std::uint32_t colour1, std::uint32_t colour2) {
