...
class A3DManager {
   ...
   public:
    ...
    inline void ListenPos(float x, float y, float z) {
        if (a3dlis) a3dlis->SetPosition3f(x, y, z);
    };
    inline void ListenRot(float head, float roll, float pitch) {
        if (a3dlis) a3dlis->SetOrientationAngles3f(head, pitch, roll);
    };
...
class A3DSource : public A3DBase {
   ...
   public:
    ...
    inline void SetPriority(float priority) {
        if (a3dsrc) a3dsrc->SetPriority(priority);
    };
    void SetMute(std::uint8_t mute);
    void Set3D(std::uint8_t is3d);
    inline void SetGain(std::uint8_t vol) {
        if (a3dsrc) a3dsrc->SetGain(((float) vol) / 255.0f);
    };
    ...
    inline void SetPositionf(float x, float y, float z) {
        if (a3dsrc) a3dsrc->SetPosition3f(x, y, z);
    };
    inline void SetPositionl(std::int32_t x, std::int32_t y, std::int32_t z) {
        if (a3dsrc) a3dsrc->SetPosition3f((float) x, (float) y, (float) z);
    };
    inline void SetRotationf(float h, float r, float p) {
        if (a3dsrc) a3dsrc->SetOrientationAngles3f(h, r, p);
    };
    inline void SetRotationl(std::int32_t h, std::int32_t r, std::int32_t p) {
        if (a3dsrc) a3dsrc->SetOrientationAngles3f((float) h, (float) r, (float) p);
    };
    inline void SetVelocityf(float x, float y, float z) {
        if (a3dsrc) a3dsrc->SetVelocity3f(x, y, z);
    };
    inline void SetVelocityl(std::int32_t x, std::int32_t y, std::int32_t z) {
        if (a3dsrc) a3dsrc->SetVelocity3f((float) x, (float) y, (float) z);
    };

    // audio properties
    inline void SetPitchf(float pitchbend) {
        if (a3dsrc) a3dsrc->SetPitch(pitchbend);
    };
    inline void SetGainf(float gain) {
        if (a3dsrc) a3dsrc->SetGain(gain);
    };
