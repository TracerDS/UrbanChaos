...
void ELEV_load_level(char* fname_level) {
    ...
    if (fname_level) {
        ...
        if (handle == FILE_OPEN_ERROR) {
            ...
            if (FileRead(handle, &version, sizeof(std::int32_t)) == FILE_READ_ERROR)
                goto file_error; // Version
            if (FileRead(handle, &flag, sizeof(std::int32_t)) == FILE_READ_ERROR)
            ...
            if (FileRead(handle, junk, sizeof(std::uint16_t)) == FILE_READ_ERROR)
...
std::int32_t ELEV_load_name(char* fname_level) {
    ...
    if (FileRead(handle, junk, sizeof(std::int32_t)) == FILE_READ_ERROR)
