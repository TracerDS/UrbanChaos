...
std::int32_t should_i_jump(Thing *darci) {
    ...
    if (x1 == x2 && z1 == z2 &&
        ...
        return WARE_which_contains(x1, z1) == WARE_which_contains(x2, z2) &&
...
// #ifdef	MIKE_I_DONT_LIKE_IT_DISKETT
    ...
    if ((p_thing->SubState == SUB_STATE_WALKING_BACKWARDS)
        ...
        || (p_thing->Velocity <= 12) || (p_thing->State == STATE_JUMPING)) {
        wDesiredRoll = 0;
    }
...
#define ADD_CHEAT_BUTTON(name, character) \
    if (BUTTON_IS_PRESSED(the_state.rgbButtons[DI_DC_BUTTON_##name])) cAdd = character
