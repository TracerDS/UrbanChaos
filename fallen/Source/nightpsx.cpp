...
std::int32_t NIGHT_load_ed_file(char* name) {
    ...
    if (FileRead(handle, &sizeof_ed_light, sizeof(std::int32_t)) != sizeof(std::int32_t))
        goto file_error;
    if (FileRead(handle, &ed_max_lights, sizeof(std::int32_t)) != sizeof(std::int32_t))
        goto file_error;
    if (FileRead(handle, &sizeof_night_colour, sizeof(std::int32_t)) != sizeof(std::int32_t))
        goto file_error;
    ...
    if (FileRead(handle, &ed_light_free, sizeof(std::int32_t)) != sizeof(std::int32_t))
        goto file_error;
    ...
    if (FileRead(handle, &flag, sizeof(std::uint32_t)) != sizeof(std::uint32_t))
        goto file_error;
    if (FileRead(handle, &amb_d3d_colour, sizeof(std::uint32_t)) != sizeof(std::uint32_t))
        goto file_error;
    if (FileRead(handle, &amb_d3d_specular, sizeof(std::uint32_t)) != sizeof(std::uint32_t))
        goto file_error;
    if (FileRead(handle, &amb_red, sizeof(std::uint32_t)) != sizeof(std::uint32_t))
        goto file_error;
    if (FileRead(handle, &amb_green, sizeof(std::uint32_t)) != sizeof(std::uint32_t))
        goto file_error;
    if (FileRead(handle, &amb_blue, sizeof(std::uint32_t)) != sizeof(std::uint32_t))
        goto file_error;
    if (FileRead(handle, &lampost_red, sizeof(std::int8_t)) != sizeof(std::int8_t))
        goto file_error;
    if (FileRead(handle, &lampost_green, sizeof(std::int8_t)) != sizeof(std::int8_t))
        goto file_error;
    if (FileRead(handle, &lampost_blue, sizeof(std::int8_t)) != sizeof(std::int8_t))
        goto file_error;
    if (FileRead(handle, &padding, sizeof(std::uint8_t)) != sizeof(std::uint8_t))
        goto file_error;
    if (FileRead(handle, &lampost_radius, sizeof(std::int32_t)) != sizeof(std::int32_t))
        goto file_error;
